...
import Link from "next/link";
import AnimatedContent from "@/components/ui/AnimatedContent";
import  products  from "@/app/lib/product"; // Import the products array
      ...
      <AnimatedContent>
        <div className="container mx-auto">
          ...
          <div className="relative px-4 md:px-8">
                ...
                {products.map((product, index) => (
                  ...
                  >
                    <div className="group relative h-96 rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-all duration-300">
                      ...
                      {/* Content */}
                      <div className="absolute bottom-0 left-0 w-full p-6 z-10">
                        <h3 className="text-white text-sm font-bold mb-2 font-serif transition-transform duration-300 group-hover:translate-y-1">
                          {product.name}
                        </h3>
                        <p className="text-gray-200 mb-4 text-sm font-sans transition-all duration-300 max-w-xs opacity-80 group-hover:opacity-100">
                          {product.description}
                        </p>
                        <div className="transform translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
            ...
            <div className="flex justify-center mt-6 space-x-2">
              {Array.from({
                length: Math.ceil(products.length - itemsToShow + 1),
              }).map((_, index) => (
                <button
                  key={index}
                  onClick={() => setCurrentIndex(index)}
